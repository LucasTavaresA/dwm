...
void bstack(Monitor *m) {
  ...
  if (n > m->nmaster)
    ...
    mh = m->wh;
  for (i = 0, mx = tx = 0, c = nexttiled(m->clients); c;
...
int containsemacsclient(pid_t p) {
...
pid_t getyoungestchild(pid_t p) {
...
/*  */
		...
		/* for (i = 0; i < LENGTH(keys); i++) */
          ...
          /* XUngrabKey(dpy, code, keys[i].mod | modifiers[j], root); */
        /* } */
	/* } */
/* } */
...
void movemouse(const Arg *arg) {
  ...
  if (!(c = selmon->sel))
    return;
  /* if (c->isfullscreen && */
      /* !c->isfakefullscreen) /\* janelas em tela cheia se movimentam com o mouse *\/ */
    /* return; */
  restack(selmon);
  ocx = c->x;
  ocy = c->y;
...
void resizemouse(const Arg *arg) {
  ...
  if (!(c = selmon->sel))
    return;
  /* if (c->isfullscreen && !c->isfakefullscreen) /\* Janela em tela cheia se movimentam com o mouse *\/ */
    /* return; */
  restack(selmon);
  ocx = c->x;
  ocy = c->y;
